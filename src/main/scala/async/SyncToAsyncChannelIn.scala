...
class SyncToAsyncChannelOut[T <: Data](A: T) extends Module {
  ...
  io.async.dual := Mux(dataEn, Dual.from(io.data), 0.U.asTypeOf(Dual(A)))
  ...
  // State 1 enable=T busy=F ack=F on clock posedge ; get data
  // State 1.5 enable=? busy=T ack=F dataEn=T
  // State 2 enable=? busy=T ack=T dataEn=T
  // State 2.5 enable=? busy=T ack=T dataEn=F
  // State 3 enable=? busy=T ack=F dataEn=F
  // State 4 enable=? busy=T ack=F dataEn=F
